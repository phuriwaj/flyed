...
- `/admin/*` (Decap CMS) — gated by the Decap Cloud OAuth flow described in
  `docs/decap-cms.md` (not owned by this document; flagged for update in audit
  B.3 because it still references CF Pages, not Workers).
...
| R-3  | Decap CMS docs (`docs/decap-cms.md`, `public/admin/README.md`) still reference Cloudflare Pages (audit B.3, B.5). Preview URL format is stale.                                                                    | Onboarding confusion after Workers migration.                          | Medium.                                           | Rewrite per audit D.3.3 and D.3.4 (out of scope for this document).                                               |
